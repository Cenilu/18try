    ...
    function watchUserPosition() {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.watchPosition(function (position) {
                ...
                // Remove previous myLocationMarker if exists
                if (myLocationMarker) {
            ...
            // Event listener for marker addition on the map
            map.on('layeradd', function (event) {
                if (event.layer instanceof L.Marker && event.layer !== myLocationMarker) {
                    // Make previous myLocationMarker null
                    if (myLocationMarker) {
                        map.removeLayer(myLocationMarker);
                        myLocationMarker = null;
                    }
    
                    // Start routing on the new marker
                    if (searchMarker && routingControl) {
                        routingControl.setWaypoints([
